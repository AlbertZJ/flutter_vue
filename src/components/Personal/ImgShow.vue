...
<div class="tail">
  <div class="tail">
    ...
    <el-button style="background-color:lightsteelblue;color:blanchedalmond"  @click="wishes">设置寄语</el-button>
<h1 style="color:blanchedalmond;">主人寄语：</h1>
  <div style="text-align: center;color:burlywood;" v-html="message.htmlContent"></div>
...
export default {
  ...
  methods: {
   wishes(){
 var _this = this;
      _this.$router.replace({ path: "/postWishes" });
   },
    logout() {
      var _this = this;
  ...
  mounted: function() {
    var _this = this;
    getRequest("/message/get").then(
      resp=>{
       // _this.$alert("resp.status");
        if(resp.status==200){
          _this.message=resp.data;
          //_this.$alert(_this.message);
        }else{
